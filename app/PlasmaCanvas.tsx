...
export function PlasmaCanvas() {
  ...
  useEffect(() => {
    ...
    gl.uniform4f(cursor, 0.0, 4.0, 0.65, 0.3);
    ...
    const draw = (now: number) => {
      resize();
      const seconds = reduceMotion ? 0 : (now - startedAt) / 1000;
      gl.uniform4f(scene, canvas.width, canvas.height, seconds * 0.86, 4.0);
    ...
    const handleVisibility = () => {
      ...
      if (!document.hidden && !reduceMotion) animationFrame = requestAnimationFrame(loop);
    };
